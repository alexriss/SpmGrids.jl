@testset "image stacks" begin
    ...
    # grid index is x,y,z, for image it is row/col (i.e. y,x), thus we use data-transposed
    @test approx(get_channel(grid, "Z", 15, 1, 3), SpmImages.get_channel(images[3], "Z").data'[1,15])
    ...
    @test get_channel(grid, "Frequency Shift", 23, 25, 4)[] ≈ -26.063005447387695
    @test approx(get_channel(grid, "Bias_", 15, 1, 3), SpmImages.get_channel(images[3], "Bias_").data'[15,1])
    ...
    @test approx(get_channel(grid, "Bias_", 15, 1, 3), SpmImages.get_channel(images[3], "Bias_").data'[15,1])
    @test get_channel(grid, "Frequency Shift", 3, 5, 4)[] ≈ -25.569202423095703
    @test get_channel(grid, "Frequency Shift", 23, 25, 4)[] ≈ -25.272993087768555
    @test approx(get_channel(grid, "Bias_", 15, 1, 3), SpmImages.get_channel(images[3], "Bias_").data'[15,1])
    ...
    @test get_channel(grid, "Frequency Shift", 23, 25, 4)[] ≈ -25.272993087768555
    @test get_channel(grid, "Phase", 23, 15, 3)[] ≈ 0.030607182532548904

    grid = load_grid(images, im -> im.z, "Z_", "m", only_overlap=true)
    @test approx(grid.center * 1e9, im2.center .- offset ./ 2)
    @test approx(grid.size * 1e9, im2.scansize .- offset)
    @test size(grid.data) == (38, 34, 92)
    @test get_channel(grid, "Phase", 1, 15, 3)[] ≈ 0.3382571041584015
